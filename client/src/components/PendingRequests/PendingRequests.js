...
function PendingRequests(props) {
    ...
    const [pendingTabStyleValues, setPendingTabStyleValues] = useState(["black", "bold", "blue"]);
    const [activeTabStyleValues, setActiveTabStyleValues] = useState(["grey", "default", "lightGray"]);
            ...
            if (i) {
                return (
                    <div>
                        ...
                        <button onClick={() => { props.deleteActiveRequest(i.reciever_mobile) }} className="bg-red-500 ml-6">Cancel</button>
                    </div>
                )
            }
        })
        ...
        <div className="h-full">
            <div className="flex justify-center">
                ...
                <span
                    ...
                    className="ml-4 text-lg text-gray-500 border-blue-200 border-b-2  cursor-default hover:text-black hover:border-black hover:cursor-pointer active-tab" style={activeTabStyle}>
                    ACTIVE
                </span>
            </div>
            ...
            <div className="h-full" style={{ display: activeDisplay }}>
                <div className="no-req h-full m-auto flex" style={{ display: noReqDisplay }}>
                    <h4 className="text-center m-auto self-center text-gray-600">There are no active requests.</h4>
                </div>
                <div>
                    {activeRequestsList}
                </div>
